...
Eigen::Matrix4d initial_guess_pre;

Eigen::Matrix3d odom_rot;
Eigen::Matrix4d odom_transform = Eigen::Matrix4d::Identity();
Eigen::Matrix4d odom_transform_pre = Eigen::Matrix4d::Identity();
...
void odomCallback(const nav_msgs::Odometry::ConstPtr msg)
{
	...
	//odom_transform = Eigen::Matrix4d::Identity();
	...
	odom_transform(3,3) = 1;

	odom_transform = odom_origin * odom_transform;

	odom_diff = odom_transform - odom_transform_pre;
	odom_diff.block(0,0,3,3) = Eigen::Matrix3d::Zero();//keep translation only
	//odom_diff.block(0,0,3,3) = Eigen::Matrix3d::Identity();
	initial_guess(0,3) = initial_guess_pre(0,3) + odom_diff(0,3);//initial_guess_pre is from last odomCallback
	//only use x from wheel_odometry
	initial_guess(3,3) = 1;
	initial_guess_pre = initial_guess;
...
void icpCallback(const sensor_msgs::PointCloud2::ConstPtr &lidar_points)
{
	...
	if(init)
	{
		...
		initial_guess_pre = odom_origin;
		odom_transform_pre = odom_origin;
		init = 0;
	}
